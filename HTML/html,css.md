...
**반응형 웹이란**? Responsive Web

**반응형 웹**이란 PC, 태블릿 PC, 스마트폰 등, 다양한 디바이스의 해상도에 맞춰 웹사이트 내 콘텐츠의 크기 및 레이아웃이 자동으로 재조정되는 것을 말합니다.
...
**1. Fluid grids **(유동형 그리드)

콘텐츠가 가지고 있는 영역 값을 고정된 px(픽셀) 값이 아닌 %(퍼센트) 값을 사용하여 구축하는 기법으로, 비율에 기초한 그리드를 기준으로 콘텐츠의 폭을 맞추는 방법을 의미합니다.

**2. Flexible images **(유동형 이미지)

유동형 그리드와 마찬가지로 고정된 px 값이 아닌 이미지의 너비 값을 %로 적용하는 기법으로, 스크린 가로 폭에 맞춰 이미지 크기가 조정되는 것을 의미합니다.

**3. Media queries **(미디어 쿼리)

각 종 디바이스의 환경 및 해상도에 알맞은 디자인을 적용할 수 있도록 하는 CSS3의 기법으로, 반응형 웹페이지를 만들 수 있습니다.

**- wrap**  **또는** **wrapper** **박스가 있는 이유**       

웹 문서 내용 전체의 크기나 배경색을 한꺼번에 조절할 수 있고 내용을 중앙에 배치하거나, 반응형웹에서는 무제한으로 늘어나는 것을 제어할 수 있다.    
...
`@media [미디어 유형] [and 또는 , 콤마] ( 조건문 ) { 실행문 }`

**min**을 사용할 때 크기(width)가 작은 순서대로 작성해야 하는 이유는 min 은 최소 그 이상이라는 뜻으로 점차 커지는 것을 의미하기 때문에 반드시 **작은 순서부터 큰 순서로 작성**해야 합니다.

반대로 **max**는 최대 또는 그 이하라는 뜻으로 점차 작아지는 것을 의미하기 때문에 max를 사용할 때는 반드시 큰 순서부터 작은 순서로 작성해야 합니다.
...
**css font-size [ vw, vh, vmax, vmin, em, rem ]** **단위**

| **화면 사이즈기준**으로 폰트사이즈 변경 |                                        |       |      | 화면 사이즈 **1024** **x** **564** |
| --------------------------------------- | -------------------------------------- | ----- | ---- | ---------------------------------- |
| vw                                      | viewport width단위 넓이 기준으로 100%  | 2vw   | 2%   | (2*1024)/100=20.48px               |
| vh                                      | viewport height단위높이기준으로 100%   | 3vh   | 3%   | (2*564)/100=16.92px                |
| vmax                                    | viewport maximum단위화면의 큰쪽기준    | 2vmax | 2%   | (2*1024)/100=20.48px               |
| vmin                                    | viewport mininum단위 화면의 작은쪽기준 | 3vmin | 3%   | (3*564)/100=16.92px                |



| em   | **부모요소 section  section {font-size:20px;}em은 상위(부모)가 기준이 된다.** | 1.2em  | 부모요소 | 20 x 1.2 = 24px |
| ---- | ------------------------------------------------------------ | ------ | -------- | --------------- |
| rem  | 최상위요소 htmlhtml {font-size:10px;}rem은 최상위 요소가 기준이 된다최상위를 뜻하는 root 의 r을 붙여줘서 rem | 1.5rem | html     | 10 x 1.5 = 15px |
